...
#pragma warning restore CA2000
    ...
    private static Stream? GetExtensionStream(ResolvedFileInfo entry, IReadOnlyList<ITheme> extensions, string path)
    {
    ...
    private async Task<string?> PromptForThemeSelectionAsync(CancellationToken cancellationToken)
    {
        ...
        var choices = listResult.Installed
            .Select(t => new ThemeChoice(t.Metadata.Name, t.IsLocal ? "local" : (t.Source?.ToString() ?? "installed")))
            .ToList();
        ...
        foreach (var file in selectedFiles.Where(f => f.Category != "group"))
        {
            ...
            try
            {
                ...
                if (file.SourceType == FileSourceType.Extension && file.ExtensionIndex >= 0 && file.ExtensionIndex < extensions.Count)
                {
                    ...
                    var extPrefix = extension.Prefix ?? string.Empty;
                    var folderName = extPrefix.Length > 0
                        ? char.ToUpperInvariant(extPrefix[0]) + extPrefix[1..]
                        : "Extension";
